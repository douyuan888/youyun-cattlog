kubelet:
    ...
    command:
        ...
        - {{ $elem }}
        {{- end }}
    image: youyun-private-harbor:9080/library/k8s:v1.7.4-rancher2
    volumes:
        - /run:/run
        - /var/run:/var/run
        - /sys:/sys:ro
...
etcd:
    image: youyun-private-harbor:9080/library/etcd:v2.3.7-13
    labels:
        {{- if eq .Values.CONSTRAINT_TYPE "required" }}
...
kube-hostname-updater:
    ...
    command:
        - etc-host-updater
    image: youyun-private-harbor:9080/library/etc-host-updater:v0.0.3
    links:
        - kubernetes
...
kubectld:
    ...
    environment:
        ...
        LISTEN: ":8091"
    image: youyun-private-harbor:9080/library/kubectld:v0.8.3
    links:
        - kubernetes
...
rancher-kubernetes-agent:
    ...
    environment:
        KUBERNETES_URL: https://kubernetes.kubernetes.rancher.internal:6443
    image: youyun-private-harbor:9080/library/kubernetes-agent:v0.6.5
    privileged: true
    volumes:
...
rancher-ingress-controller:
    image: youyun-private-harbor:9080/library/lb-service-rancher:v0.7.10
    labels:
        {{- if eq .Values.CONSTRAINT_TYPE "required" }}
        io.rancher.scheduler.affinity:host_label: orchestration=true
        {{- end }}
...
rancher-kubernetes-auth:
    image: youyun-private-harbor:9080/library/kubernetes-auth:v0.0.8
    labels:
        {{- if eq .Values.CONSTRAINT_TYPE "required" }}
        io.rancher.scheduler.affinity:host_label: orchestration=true
        {{- end }}
